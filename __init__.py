...
def edit_triangulate_to_quads(obj):
    ...
    # Using pi (180Â°) for both thresholds allows maximum quad conversion
    try:
...
def add_high_suffix_main():
    ...
    for obj in high.objects:
        ...
        renamed_count += 1
        print(f"Renamed: '{original_name}' â†’ '{obj.name}'")
...
def convert_main():
    ...
    if not high:
        print(
            f"ERROR: Collection '{HIGH_COLL}' not found. Please create a '{HIGH_COLL}' collection with your high-poly objects.")
        return
